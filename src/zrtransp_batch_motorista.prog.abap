*&---------------------------------------------------------------------*
*& Report ZRTRANSP_BATCH_MOTORISTA
*&---------------------------------------------------------------------*
*& Carga Batch Input para dados de cadastro de motorista em massa
*&  - Tabela: ZTMOTORISTAS
*&  - Transaction: ZMTMOTORISTA
*&  - Programa: SAPMZTRANSPORTE
*&  - Mapeamento BDC: CAD_MOTORIST
*&---------------------------------------------------------------------*

REPORT zrtransp_batch_motorista NO STANDARD PAGE HEADING.
...
TYPES: BEGIN OF ty_format_file,
         motorista_id    TYPE ztmotoristas-motorista_id,
         nome	           TYPE ztmotoristas-nome,
         cpf             TYPE ztmotoristas-cpf,
         cnh             TYPE ztmotoristas-cnh,
         validade_cnh(8) TYPE c,        " Original data type DATS
         status          TYPE ztmotoristas-status,
  ...
  TRY .
      ...
      IF sy-subrc IS INITIAL.
        ...
        LOOP AT lt_csv INTO wa_csv.
          SPLIT wa_csv AT ';' INTO wa_format_file-motorista_id
                                   wa_format_file-nome
                                   wa_format_file-cpf
                                   wa_format_file-cnh
                                   wa_format_file-validade_cnh
...
FORM f_create_bdc .
  ...
  LOOP AT lt_format_file INTO wa_format_file.
    ...
    PERFORM f_mount_screen USING 'SAPLZGRF_TRANSPORTES' '0009'.
    PERFORM f_mount_data   USING 'BDC_CURSOR'           'ZTMOTORISTAS-NOME(01)'.
    PERFORM f_mount_data   USING 'BDC_OKCODE'           '=NEWL'.

    PERFORM f_mount_screen USING 'SAPLZGRF_TRANSPORTES' '0010'.
    PERFORM f_mount_data   USING 'BDC_CURSOR'           'ZTMOTORISTAS-STATUS'.
    PERFORM f_mount_data   USING 'BDC_OKCODE'           '=SAVE'.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-MOTORISTA_ID' wa_format_file-motorista_id.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-NOME'         wa_format_file-nome.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-CPF'          wa_format_file-cpf.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-CNH'          wa_format_file-cnh.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-VALIDADE_CNH' wa_format_file-validade_cnh.
    PERFORM f_mount_data   USING 'ZTMOTORISTAS-STATUS'       wa_format_file-status.


    PERFORM f_mount_screen USING 'SAPLZGRF_TRANSPORTES' '0010'.
    PERFORM f_mount_data   USING 'BDC_CURSOR'           'ZTMOTORISTAS-NOME'.
...
FORM f_open_folder .
  ...
  TRY .
      CALL FUNCTION 'BDC_OPEN_GROUP'
        EXPORTING
          client              = sy-mandt
          group               = 'CARGA_MOTOR'   " Nome da pasta
...
FORM f_insert_bdc .
  ...
  TRY .
      CALL FUNCTION 'BDC_INSERT'
        EXPORTING
          tcode            = 'ZMTMOTORISTA'  " Código da transaction
        TABLES
